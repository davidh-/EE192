...
volatile float refVel = 2.2; // Corresponds to m/s
const float refVel_strt = 2.2; // Corresponds to m/s
const float refVel_turn = 2.0; // Corresponds to m/s
const float dt = 0.010; //PI interrupt in SECONDS 50 Hz
const float Kp_vel = 0.17; //Proportional Gain
...
const float Kp_steer = 5.7; //in degrees of steering per pixel error
const float Kd_steer = 0.7; //in degrees/s of steering change per pixel error
const int refSteer = 64;
...
void control() {
	...
	if ((pos>(middle + 150)) || (pos<(middle - 150))) {
		refVel = refVel_turn;
	} else {
		refVel = refVel_strt;
	}
